...
import sys, os
import datetime
import sdnconfig as configuration
...
def cmdarg(argv):
    try:
        ...
        print '''python sdn_topology.py --switch_count <switch count> \
                 --controller_node_ip <odl controller node ip>'''
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print '''python sdn_topology.py --switch_count <switch count> \
...
def vx_sdn_set_controller():
    session_ip = configuration.emulation_host_ip
    username = "admin"
    password = "admin"
    controller_ip = controller_node_ip
    controller_name = configuration.controllername
    build_version = configuration.buildversion
    openflow_mode = configuration.openflowmode
    openflow_port = configuration.controllerport
    openflow_version = configuration.openflowversion
    ...
    if message:
        cid = message.split(':')[1]
        controller_id_created = cid.strip()
        return controller_id_created
