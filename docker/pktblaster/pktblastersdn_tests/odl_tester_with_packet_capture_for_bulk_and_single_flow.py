...
print '''\nThis script calculate the \"Proactive Path Provisioning Time\" \
with bulk flows install from Northbound insterface in single REST request.\n'''

print "\nNote:"
print '''      Make sure the SDN controller and mininet is \
running before starting the test.'''
print '''      Use --fpr and --bulk_add option to send bulk flows \
in single REST request.'''
...
print "Debug: " + str(flow_template)

odl_node_url = '/restconf/config/opendaylight-inventory:nodes/node/'


print "Step 1: Initiating packet capture at the NB and SB controller interface.\n"

##Starting the packer capture from northbound and southbound interface.

def work1():
    '''
    @description: tshark capture the "HTTP POST" request message from northbound interface
    @return: tshark output write into a file
    '''
...
def work2():
    '''
    @description: tshark capture the "FLOW_MOD" message from southbound interface.
    @return: tshark output write into a file
    '''
    global inargs
...
def main(*argv):
    ...
    parser.add_argument('--controller_host_ip', default='127.0.0.1',
        ...
        for i in range(int(in_args.threads)):
            thr = threading.Thread(target=_wt_request_sender, args=(i, preparefnc),
                                   kwargs={"inqueue": sendqueue, "exitevent": exitevent,
                                           "controllers": [in_args.controller_host_ip], "restport": in_args.port,
 ...
 # lets print some stats
    ...
    with Timer() as t:
        for i in range(rounds):
            reported_flows = len(get_flow_ids(controller=in_args.controller_host_ip, port=in_args.port))
    ...
    with Timer() as tmr:
        if in_args.bulk_delete:
            url = 'http://' + in_args.controller_host_ip + ':' + in_args.port
...
##Checking the minimum and maximum provisioning time
        ...
        minimum = min(vol)
        maximum = max(vol)
    result_file = open(filename3, "w")

    print "        Proactive Flow Provisioning Time:", "min / max / avg:", minimum, "/", maximum, "/", AvgTime, "milliseconds"
    print "\n"

### Output of the test is printed in html file

    result_file.write("<html>\n")
    result_file.write("<head>\n")
    result_file.write("<body>\n")
    result_file.write("<b> Proactive Flow Provisioning Time: </b> <br>" + "min / max / avg: " +str(minimum)+" / " +str(maximum)+ " / " +str(AvgTime)+" milliseconds <br> \n")
